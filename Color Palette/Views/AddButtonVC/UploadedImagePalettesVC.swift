...
class UploadedImagePalettesVC: UIViewController {
    ...
    var image: UIImage?
    private var selectedIndexPath: IndexPath?
    private var hasAdjustedThisSession = false // Track if adjustment has been counted this session
    ...
    override func viewDidLoad() {
        ...
        exitButtonView.clipsToBounds = true
        sliderResultContainers()
        hasAdjustedThisSession = false // Reset for new session
    ...
    @IBAction func savePalettes(_ sender: UIButton) {
        ...
        if CoreDataManager.shared.isSingleColorSaved(name: hexCode) {
            ...
            showToast(message: "Color already saved!")
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                self.finalHexaCode.text = hexCode
            }
            return
        }
        
        let savedColors = CoreDataManager.shared.fetchSingleColors()
        if !isPremiumUser() && savedColors.count >= 3 { // Limit to 3 saves for non-premium users
            NotificationCenter.default.post(name: CoreDataManager.showPremiumScreenNotification, object: nil)
            showToast(message: "Upgrade to save more colors!")
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                self.finalHexaCode.text = hexCode
            }
        } else {
    ...
    @IBAction func copyButtonPressed(_ sender: UIButton) {
        if UserLimitsManager.canDownloadPalette() {
            guard let hexCode = finalHexaCode.text else { return }
            UserLimitsManager.incrementDownloadCount()
            ...
            showToast(message: UserLimitsManager.limitReachedMessage(for: .download))
            presentProScreen()
        }
    }
    ...
    private func canAdjust() -> Bool {
        if isPremiumUser() {
            return true
        }
        if UserLimitsManager.canAdjustPalette() {
            if !hasAdjustedThisSession {
                UserLimitsManager.incrementAdjustmentCount()
                hasAdjustedThisSession = true
            }
            return true
        } else {
            showToast(message: UserLimitsManager.limitReachedMessage(for: .adjustment))
            presentProScreen()
            return false
        }
    }
    
    private func presentProScreen() {
...
extension UploadedImagePalettesVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "CollectionViewCellInUploadedImagePalettesVC", for: indexPath) as? CollectionViewCellInUploadedImagePalettesVC else {
            fatalError("Unable to dequeue CollectionViewCellInUploadedImagePalettesVC")
        }
        let color = extractedColors[indexPath.item]
        cell.colorView.backgroundColor = color
        cell.hexaCodeLabel.text = color.toHexString()
